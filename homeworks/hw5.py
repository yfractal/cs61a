...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    incorrect_passwords = []
    count_incorrect_password = 0

    def withdraw(withdraw_money,pw):
        nonlocal balance,password,count_incorrect_password
        if count_incorrect_password >= 3:
            return "Your account is locked. Attempts: " + str(incorrect_passwords)
        elif pw != password:
            count_incorrect_password += 1
...
def make_joint(withdraw, old_password, new_password):
    ...
    1,check the old password is right or not
    2,return a joint_with_daw()
       if the password is a new password,use withdraw(money,old_password)
       else withdraw(money,password)
    ...
    # initialize
    new_passwords = [] # new passward is in here

    check_password = withdraw(0,old_password)

    def save_new_passowrd(new_password):
        new_passwords.append(new_password)

    if type(check_password) != int: #if the old password is not wright ,the with_draw will return a message ,not a int 
        return check_password
    else:
        save_new_passowrd(new_password)

    def make_joint_h(withdraw_money,password):
        nonlocal old_password        
        if password in new_passwords:  
            return withdraw(withdraw_money,old_password)
        else:
            return withdraw(withdraw_money,password)
